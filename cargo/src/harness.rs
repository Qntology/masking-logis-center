...
impl DefaultHarness {
    ...
    pub fn clean_html(&self, html: &str, include_images: bool) -> String {
        ...
        self.process_node_as_pug(document.root_element().clone(), &mut cleaned_text, include_images);
    ...
    fn process_node_as_pug(&self, element_ref: scraper::ElementRef, output: &mut String, include_images: bool) {
        ...
        for child in element_ref.children() {
            match child.value() {
                Node::Element(_) => {
                    if let Some(child_ref) = scraper::ElementRef::wrap(child) {
                        self.process_node_as_pug(child_ref, output, include_images);
                    }
                }
                Node::Text(text) => {
...
impl Harness for DefaultHarness {
    fn normalize(&self, raw_input: &str) -> Result<State, anyhow::Error> {
        let cleaned_html = self.clean_html(raw_input, true);
