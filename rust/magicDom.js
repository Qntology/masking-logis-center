(() => {
	...
	const STYLE_ID = 'super-mw-style';
	const PANEL_ID = 'super-mw-panel';
	const H_CLASS = 'super-mw-highlight';
	const A_CLASS = 'super-mw-ancestor';

	document.getElementById(PANEL_ID)?.remove();
	document.getElementById(STYLE_ID)?.remove();
	document.querySelectorAll('.' + H_CLASS).forEach(e => e.classList.remove(H_CLASS));
	document.querySelectorAll('.' + A_CLASS).forEach(e => e.classList.remove(A_CLASS));

	const style = document.createElement('style');
	style.id = STYLE_ID;
	style.textContent = `
		.${H_CLASS} { outline: 2px solid rgba(59,130,246,0.95) !important; background: rgba(59,130,246,0.06) !important; }
		.${A_CLASS} { outline: 3px dashed rgba(234,88,12,0.95) !important; background: rgba(234,88,12,0.04) !important; }
		#${PANEL_ID} { position: fixed; right: 12px; top: 12px; z-index:2147483647; width:340px; font-family:Arial,sans-serif; font-size:13px;
			background: rgba(18,18,18,0.9); color:#fff; padding:10px; border-radius:8px; box-shadow:0 12px 30px rgba(0,0,0,0.45); }
		#${PANEL_ID} .row{ margin-top:8px; display:flex; gap:6px; align-items:center; }
		#${PANEL_ID} .small{ font-size:12px; color:#d1d5db; }
		#${PANEL_ID} button{ margin-top:6px; padding:6px; border-radius:6px; border:none; cursor:pointer; }
		#${PANEL_ID} .btn-blue{ background:#0ea5e9; color:#fff; flex:1; }
		#${PANEL_ID} .btn-red{ background:#ef4444; color:#fff; flex:1; }
		#${PANEL_ID} select, #${PANEL_ID} input[type=number], #${PANEL_ID} input[type=range] { background:#111827; color:#fff; border:1px solid #374151; padding:4px 6px; border-radius:6px; }
		#${PANEL_ID} .log{ max-height:160px; overflow:auto; margin-top:8px; color:#9ca3af; font-size:12px; white-space:pre-wrap; }
	`;
	...
	panel.innerHTML = `
		<div style="display:flex;justify-content:space-between;align-items:center">
			<div><b>Super MagicWand v1.4</b></div>
			<div class="small">Mode: <span id="smw-mode-label">auto</span></div>
		</div>
		<div class="small">Click element → auto detect structural block or list parent (active tolerant)</div>
		<div class="row small">Threshold: <span id="smw-th-val">60</span></div>
		<input id="smw-th" type="range" min="10" max="95" value="60" />
		<div class="row">
			<select id="smw-force">
				<option value="auto">Auto detect</option>
				<option value="ul-li">Force: UL/OL → li group</option>
				<option value="dl">Force: DL → dt/dd pairs</option>
				<option value="table-row">Force: TABLE → row</option>
				<option value="table-col">Force: TABLE → column</option>
				<option value="table-table">Force: TABLE → entire table</option>
				<option value="div-list">Force: DIV list/grid</option>
				<option value="list-parent">Force: Entire list parent</option>
			</select>
			<input id="smw-topk" type="number" value="100" min="5" max="1000" style="width:84px" />
		</div>
		<div class="row">
			<button id="smw-copy" class="btn-blue">Copy XPaths</button>
			<button id="smw-clear" class="btn-red">Clear</button>
		</div>
		<div class="log" id="smw-log">Initialized. Click any element to select structural block.</div>
	`;
	document.body.appendChild(panel);

	const thInput = panel.querySelector('#smw-th');
	const thVal = panel.querySelector('#smw-th-val');
	const forceSelect = panel.querySelector('#smw-force');
	const topkInput = panel.querySelector('#smw-topk');
	const logEl = panel.querySelector('#smw-log');
	const modeLabel = panel.querySelector('#smw-mode-label');

	thInput.addEventListener('input', () => thVal.textContent = thInput.value);
	forceSelect.addEventListener('change', () => modeLabel.textContent = forceSelect.value);
	...
	function fallbackCopyTextToClipboard(text) {
		...
		textArea.value = text;
		textArea.style = `position:absolute; left:-9999999px; top:-9999999px; z-index:-1;`
		document.body.appendChild(textArea);
		textArea.focus();
		textArea.select();
		try {
			document.execCommand('copy');
			log('XPaths copied to clipboard (fallback)');
		} catch (err) {
			log('Fallback copy failed: ' + err);
		}
	...
	function cleanClassList(el) {
		...
		return Array.from(el.classList).filter(c => !ignore.includes(c.toLowerCase())).sort();
	}
	...
	function detectDivListParent(el) {
		...
		for (let i = 0; i < 6 && cur; i++) {
			...
			if (entries.length && entries[0][1] / children.length > 0.6) return cur;
			cur = cur.parentElement;
		}
		return null;
	}
	...
	function selectListParent(target) {
		return target.closest('ul,ol,dl,table') || detectDivListParent(target);
	}

	function collectDlPairs(dl) {
		const items = [];
		let cur = null;
		const children = Array.from(dl.children);
		for (const ch of children) {
			if (ch.tagName.toLowerCase() === 'dt') {
				cur = {
					dt: ch,
					dds: []
				};
				items.push(cur);
			} else if (ch.tagName.toLowerCase() === 'dd') {
				if (!cur) {
					cur = {
						dt: null,
						dds: []
	...
	function getCellIndex(cell) {
		...
		if (!row) return -1;
		const cells = Array.from(row.children).filter(n => /td|th/i.test(n.nodeName));
		return cells.indexOf(cell);
	}

	function collectTableColumn(table, colIndex) {
		return Array.from(table.querySelectorAll('tr')).flatMap(r => Array.from(r.children).filter(c => /td|th/i.test(c.nodeName)).filter((c, i) => i === colIndex));
	}

	function collectListItems(list, patternLi) {
		const lis = Array.from(list.querySelectorAll('li'));
		if (!patternLi) return lis;
		const sig = patternLi.tagName.toLowerCase() + '|' + cleanClassList(patternLi).slice(0, 4).join(',');
		return lis.filter(li => (li.tagName.toLowerCase() + '|' + cleanClassList(li).slice(0, 4).join(',')) === sig);
	}
	...
	function findCommonAncestor(elements) {
		...
		for (let i = 0; i < minLen; i++) {
			...
			if (paths.every(p => p[i] === node)) ancestor = node;
			else break;
		}
		return ancestor;
	}
	...
	const handler = async function(e){
		...
		if(target.id == "smw-copy"){
			try {
				...
				if(navigator.clipboard && navigator.clipboard.writeText){
					navigator.clipboard.writeText(body).then(()=>log('XPaths copied to clipboard'))
					.catch(()=>fallbackCopyTextToClipboard(body));
				}else{
		...
		lastSelected = [];
		lastInfo = null;
		const forced = forceSelect.value === 'auto' ? null : forceSelect.value;
		let chosen = [],
			info = {};

		if (forced === 'list-parent') {
			const lp = selectListParent(target);
			if (lp) {
				chosen = [lp];
				info = {
					type: 'list-parent',
			...
			// auto / other force modes → 1,2,3,4,5 구조
			...
			const divList = detectDivListParent(target);

			if (dl) {
				const pairs = collectDlPairs(dl);
				let matched = [];
				for (const p of pairs) {
					if (p.dt && p.dt.contains(target)) matched = [p.dt, ...p.dds];
					for (const d of p.dds)
						if (d.contains(target)) matched = [p.dt, ...p.dds];
				}
				if (!matched.length) matched = [dl];
				chosen = matched;
				info = {
					type: 'dl-pair',
					count: chosen.length
				...
				if (forced === 'table-col') {
					chosen = collectTableColumn(table, getCellIndex(td));
					info = {
						type: 'table-col',
						count: chosen.length
					};
				} else if (forced === 'table-table') {
					chosen = Array.from(table.querySelectorAll('td,th'));
					info = {
						type: 'table-table',
						count: chosen.length
					};
				} else {
					chosen = Array.from(tr.querySelectorAll('td,th'));
					info = {
						type: 'table-row',
						count: chosen.length
					};
				...
				chosen = collectListItems(list, li);
				info = {
					type: 'li-group',
					count: chosen.length
				};
			} else if (divList) {
				chosen = Array.from(divList.children);
				info = {
					type: 'div-list',
					count: chosen.length
				};
		...
		chosen = Array.from(new Set(chosen.filter(Boolean).filter(visible)));
		// 클릭한 요소가 visible 하다면 무조건 첫 번째
		let first = chosen.find(el => el.contains(target)) || chosen[0];
		chosen = first ? [first, ...chosen.filter(e => e !== first)] : [];

		const topK = Math.max(5, Math.min(1000, Number(topkInput.value) || 100));
		if (chosen.length > topK) {
			chosen = chosen.slice(0, topK);
			info.note = `trimmed-to-topK:${topK}`;
		}
		...
		lastSelected = chosen;
		lastInfo = info;
		chosen.forEach(el => el.classList.add(H_CLASS));
		const ancestor = findCommonAncestor(chosen);
		if (ancestor) ancestor.classList.add(A_CLASS);
		log(`Selected type=${info.type} count=${chosen.length} ${info.note?info.note:''}`);
	};
