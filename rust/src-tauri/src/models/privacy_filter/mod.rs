...
impl PrivacyFilterModel {
    ...
    pub fn load(model_dir: &Path, device: &Device) -> Result<Self> {
        ...
        // 🚀 [정밀도 튜닝] 배경(O)에서 마스킹 시작(B/S)으로 전환될 때의 바이어스를 -3.5로 완화하여 탐지율을 높입니다.
        viterbi_config.transition_bias_background_to_start = -3.5; 

        // 🚀 배경 유지 바이어스를 1.0으로 낮추어 탐지 유연성을 확보합니다.
        viterbi_config.transition_bias_background_stay = 1.0;

        // 🚀 마스킹 중(B -> I, I -> I)에 가중치를 주어 끊김 없는 마스킹을 유도합니다.
        viterbi_config.transition_bias_inside_to_continue = 8.0;
        viterbi_config.transition_bias_inside_to_end = 5.0;
    ...
    pub fn predict(&self, text: &str) -> Result<Vec<PrivacySpan>> {
        ...
        let empty_boosts = std::collections::HashMap::new();
        self.predict_with_context(text, &empty_map, &empty_boosts)
    }

    pub fn predict_with_context(
        &self, 
        text: &str,
        // 🚀 [Semantic Guidance] bias.json과 계산된 시맨틱 유사도 맵을 직접 수용합니다.
        semantic_scores: &std::collections::HashMap<String, Vec<f32>>, 
        // 🌟 [Dynamic Logit Biasing] 
        target_boosts: &std::collections::HashMap<&str, f32>,
    ) -> Result<Vec<PrivacySpan>> {
        if text.trim().is_empty() {
            return Ok(vec![]);
        }

        println!("[PrivacyFilterModel] 🔍 입력 텍스트 분석 시작: {}", text);
        ...
        let token_strings: Vec<String> = tokens.get_tokens().iter().map(|s| s.to_string()).collect();

        let label_path = viterbi::viterbi_decode(
            &all_logits_data, 
            s, 
            num_labels, 
            &self.viterbi_config, 
        ...
        let spans = viterbi::extract_spans(&label_path, &all_logits_data, num_labels, &label_list, &token_strings, tokens.get_offsets(), text);
        println!("[PrivacyFilterModel] ✅ 분석 결과 Spans ({}개): {:?}", spans.len(), spans);
