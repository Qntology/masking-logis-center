...
- **Operational Modes**: `balanced`, `speed`, `deep`, `autopilot`, `ralph`, `ultrawork`, `branching` (for sub-channel event delegation).
- **Branching & Pruning**:
  - `Branching`: Spawn ephemeral sub-channels for event-based tasks.
  - `Pruning`: Sub-channel results merge into Trunk; channels are pruned.
